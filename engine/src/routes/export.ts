...
router.post('/api/viewport', async (req: Request, res: Response) => {
  try {
    ...
    // Persist viewport on the canvas server (source of truth for get_resource:'scene').
    const vpCanvas = getCanvas(getCanvasId(req));
    if (zoom !== undefined) vpCanvas.viewport.zoom = zoom;
    if (offsetX !== undefined) vpCanvas.viewport.x = offsetX;
    if (offsetY !== undefined) vpCanvas.viewport.y = offsetY;
