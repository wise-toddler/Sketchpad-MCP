...
// Middleware
// Hardened: no wide-open CORS. The engine is only called server-side by the
// FastAPI gateway (same pod), so cross-origin browser access is disabled.
app.use(cors({ origin: false }));
app.use(express.json({ limit: '10mb' }));

// Shared-secret guard: every /api/* call must present the gateway's secret.
// This locks down the previously unauthenticated DELETE /api/elements/clear and
// all other element mutations from anything but the trusted gateway.
const ENGINE_SHARED_SECRET = process.env.ENGINE_SHARED_SECRET;
app.use((req: Request, res: Response, next: NextFunction) => {
  if (process.env.NODE_ENV === 'test') return next();  // route tests hit the app directly
  if (!req.path.startsWith('/api')) return next(); // /health, static, /canvases stay open
  if (!ENGINE_SHARED_SECRET) return next();          // dev fallback when unset
  if (req.headers['x-engine-secret'] === ENGINE_SHARED_SECRET) return next();
  return res.status(401).json({ success: false, error: 'Unauthorized: invalid or missing engine secret' });
});
