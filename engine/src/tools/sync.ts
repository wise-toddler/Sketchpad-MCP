...
export async function createElementOnCanvas(elementData: ServerElement): Promise<ServerElement | null> {
  ...
  // Report failure (null) when the canvas sync did not succeed — matches
  // updateElementOnCanvas. Previously this returned the input data on failure,
  // so callers printed "✅ Synced" even when the canvas write failed.
...
export async function batchCreateElementsOnCanvas(elementsData: ServerElement[]): Promise<ServerElement[] | null> {
  ...
  // Report failure (null) when the canvas sync did not succeed, instead of
  // echoing the input array back and masking the error as success.
