...
import { normalizePoints, convertTextToLabel, sanitizeFilePath, expandLabelPosition } from '../helpers.js';
  ...
  try {
    switch (name) {
      case 'create_element': {
        ...
        // Shared labelPosition expansion (helpers.expandLabelPosition) — identical
        // to batch_create_elements so the two paths never drift. A non-center label
        // becomes [shape, free-standing text]; otherwise the shape's text is folded
        // into a label by convertTextToLabel.
        const toCreate = expandLabelPosition(element).map(convertTextToLabel);
        ...
        logger.info('Element created via MCP and synced to canvas', {
          id, type: element.type, count: canvasElements.length
        });
      ...
      case 'get_resource': {
        ...
        switch (resource) {
          case 'scene': {
            // Read scene state from the canvas server (stateless-proxy safe)
            const response = await fetch(withCanvasId(`${EXPRESS_SERVER_URL}/api/scene`));
            if (!response.ok) {
              throw new Error(`HTTP server error: ${response.status} ${response.statusText}`);
            }
          ...
          case 'theme': {
            ...
            const sceneData = await response.json() as { theme: string };
            result = { theme: sceneData.theme };
            break;
          }
          default:
      ...
      case 'ungroup_elements': {
        ...
        try {
          // Source of truth = element.groupIds on the canvas server (survives the
          // stateless MCP proxy). Find all elements that belong to this group,
          // instead of the old in-process sceneState.groups map (always empty
          // across MCP calls, which broke ungroup entirely).
          const response = await fetch(withCanvasId(`${EXPRESS_SERVER_URL}/api/elements`));
          if (!response.ok) {
            throw new Error(`HTTP server error: ${response.status} ${response.statusText}`);
          }
          ...
          const groupMembers = allElements.filter(el => (el.groupIds || []).includes(groupId));

          if (groupMembers.length === 0) {
            throw new Error(`Group ${groupId} not found`);
          }

          // Remove only this specific groupId from each member, preserving others.
          const updatePromises = groupMembers.map(async (el) => {
            const updatedGroupIds = (el.groupIds || []).filter(gid => gid !== groupId);
            return await updateElementOnCanvas({ id: el.id, groupIds: updatedGroupIds });
          });
          ...
          logger.info('Ungrouping elements', { groupId, count: successCount });

          const result = { groupId, ungrouped: true, elementIds: groupMembers.map(e => e.id), successCount };
          return {
            content: [{ type: 'text', text: JSON.stringify(result, null, 2) }]
          };
        } catch (error) {
      ...
      case 'batch_create_elements': {
        ...
        // Shared labelPosition expansion (helpers.expandLabelPosition), identical to
        // create_element. Non-center labels expand into shape + free-standing text;
        // convertTextToLabel folds remaining shape text into a label.
        const expandedElements = built.flatMap(expandLabelPosition).map(convertTextToLabel);
